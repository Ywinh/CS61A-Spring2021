...
def more_boar(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    def more_boar_2num(score):
        """"score>=10  can get into this function,change it into a 2num"""
        if score < 100:
            return score
        while (score // 100) != 0:
            score //= 10
    ...
    def compare_score(score_1, score_2):
        if score_1 < 10 and score_2 < 10:
            return False
        while score_1 or score_2:
            if (score_1 % 10) >= (score_2 % 10):
                return False
            score_1 //= 10
            score_2 //= 10
        return True
    player_score = more_boar_2num(player_score)
    opponent_score = more_boar_2num(opponent_score)
    res = compare_score(player_score, opponent_score)
    return res
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            if more_boar(score0, score1) is False:
                who = next_player(who)
        else:
            score1 += take_turn(strategy1(score1, score0), score0, dice, goal)
            if more_boar(score1, score0) is False:
                who = next_player(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        running_now = score-last_score
        if running_now > running_h:
            running_h = running_now
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    i = 1
    while i <= 10:
        now = make_averaged(roll_dice, trials_count)(i, dice)
        if now >= average_last:
            average_last = now
            res = i
        i += 1
